...
contract NFTPotionAuction is Ownable, Pausable, INFTPotionWhitelist, IStructureInterface {
    ...
    */
    ...
    /**
        Function used to sort the linked list of bids

        @param id The ID of the node being sorted in the linked list
        
        @return The value assigned to that node

        @dev id is the encoded bid as generated by _encodeBid. By shifting it right by 64 bits, we
        get the pricePerToken + bidId. Because bidId is decremental starting at type(uint64).max the
        composite value makes the linked list sort the bids first by pricePerToken and then by bidId
        giving priority to earlier bids with the same price
     */
